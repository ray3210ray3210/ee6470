...
void Testbench::fetch_result() {
  ...
  sc_time pre_sample_time(0,SC_NS);
  ...
  for (y = 0; y != height; ++y) {
    for (x = 0; x != width; ++x) {
...
#endif
			int red = result.range(7,0);
      int green = result.range(15,8);
      int blue = result.range(23,16);
			////int result_red = (int)(std::sqrt(red));
      ...
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 2) = red;
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 1) = green;
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 0) = blue;
      //if(x==23&&y==6) printf("%d %d %d rgb-> %d \n", red,green,blue, result);
    ...
    unsigned long average_latency = total_latency / num_pixels;
    esc_log_latency( "dut", average_latency, "average_latency" );
    cout << "Testbench sink thread read " << num_pixels << " pixels. Average latency " << average_latency << "." << endl;
