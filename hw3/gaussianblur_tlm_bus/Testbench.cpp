    ...
    for (x = 0; x != width; ++x) {
      ...
      for (v = -yBound; v != yBound + adjustY; ++v) {   //-1, 0, 1
        for (u = -xBound; u != xBound + adjustX; ++u) { //-1, 0, 1
          ...
          mask[2] = 0xff;
          mask[3] = 0;
          initiator.write_to_socket(GAUSSIANBLUR_MM_BASE + GAUSSIANBLUR_FILTER_R_ADDR, mask,
                                    data.uc, 4);
          if((y==0&&x<6)||(y==(height-1)&&x>(width-6)))
            cout << "input: Now at " << sc_time_stamp() << endl; //print current sc_time
      ...
      int output_num=0;
      while(!done){
        initiator.read_from_socket(GAUSSIANBLUR_MM_BASE + GAUSSIANBLUR_FILTER_CHECK_ADDR, mask, data.uc, 4);
        output_num = data.sint;
        if(output_num>0) done=true;
      }
      wait(10 * CLOCK_PERIOD, SC_NS);

      initiator.read_from_socket(GAUSSIANBLUR_MM_BASE + GAUSSIANBLUR_FILTER_RESULT_ADDR, mask,
                                 data.uc, 4);
      red = data.sint;
      if(x==6 &&y==6)  printf("----%d %d %u", data.uc[0], data.sint,data.sint);
      done=false;
      output_num=0;
      while(!done){
        initiator.read_from_socket(GAUSSIANBLUR_MM_BASE + GAUSSIANBLUR_FILTER_CHECK_ADDR, mask, data.uc, 4);
      ...
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 2) = red;//red;
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 1) = green;//green;
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 0) = blue;//blue;
