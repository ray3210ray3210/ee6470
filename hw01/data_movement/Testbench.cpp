...
void Testbench::do_gaussianblur() {
  ...
  unsigned char R[256], G[256], B[256]; // color of R, G, B
  //--------------------------------------------------------
  int red, green, blue;

  o_rst.write(false);
  o_rst.write(true);
  for (y = 0; y != (height+MASK_Y-1); ++y) {
    //-----------------------modify---------------------------  
    if(y<height) {
      for (x = 0; x != width; ++x) {
        R[x] = *(source_bitmap +
    ...
    if(y>=OFFSET_SHIFT_ROW) {
      for (x = 0; x != width; ++x) {
        ...
        //cout << "Now at " << sc_time_stamp() << endl; //print current sc_time
          ...
          //  y-OFFSET_SHIFT_ROW  is to elimate the offset of moving row
        *(target_bitmap + bytes_per_pixel * (width * (y-OFFSET_SHIFT_ROW) + x) + 2) = red;      
        *(target_bitmap + bytes_per_pixel * (width * (y-OFFSET_SHIFT_ROW) + x) + 1) = green;
        *(target_bitmap + bytes_per_pixel * (width * (y-OFFSET_SHIFT_ROW) + x) + 0) = blue;
        //if ((y>250 && x==153)||(y<6&&x==153)) printf("pixel= %d, col = %d***", red, y);
