  ...
  for(int i = 0; i < width; i++){
    for(int j = 0; j < height; j++){
      ...
      blue = (data).uc[2];
        ...
        *(target_bitmap + bytes_per_pixel * (width * i + j) + 2) = red;
        *(target_bitmap + bytes_per_pixel * (width * i + j) + 1) = green;
        *(target_bitmap + bytes_per_pixel * (width * i + j) + 0) = blue;
