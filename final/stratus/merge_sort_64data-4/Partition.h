...
SC_MODULE( Partition )
{
	...
	cynw_p2p < output_t >::out	o_data_lv0_p2;	// The output port
	cynw_p2p < output_t >::out	o_data_lv0_p3;	// The output port
	...
	, o_data_lv0_p2( "o_data_lv0_p2" )
	, o_data_lv0_p3( "o_data_lv0_p3" )
	{
#if defined(FLAT)
		HLS_FLATTEN_ARRAY(p0_array);
		HLS_FLATTEN_ARRAY(p1_array);
		HLS_FLATTEN_ARRAY(p2_array);
		HLS_FLATTEN_ARRAY(p3_array);
#endif
#if defined(MEM)
    	HLS_MAP_TO_MEMORY( p0_array );
    	HLS_MAP_TO_MEMORY( p1_array );
    	HLS_MAP_TO_MEMORY( p2_array );
    	HLS_MAP_TO_MEMORY( p3_array );
#endif
#if defined(REG)
    	HLS_MAP_TO_REG_BANK( p0_array );
    	HLS_MAP_TO_REG_BANK( p1_array );
    	HLS_MAP_TO_REG_BANK( p2_array );
    	HLS_MAP_TO_REG_BANK( p3_array );
		...
		o_data_lv0_p2.clk_rst( clk, rst );
		o_data_lv0_p3.clk_rst( clk, rst );
	}

	// Declare the array
	sc_uint< 32 > p0_array[DATA_NUM_L1];		//DATA_NUM x32
	sc_uint< 32 > p1_array[DATA_NUM_L1];
	sc_uint< 32 > p2_array[DATA_NUM_L1];
	sc_uint< 32 > p3_array[DATA_NUM_L1];
