...
struct MergeLV3_p0 : public sc_module {
  tlm_utils::simple_target_socket<MergeLV3_p0> tsock;

    //---FIFO IN---
  sc_fifo<unsigned int> i_data_lv2_p0;
  sc_fifo<unsigned int> i_data_lv2_p1;


//     //---FIFO OUT---
  sc_fifo<unsigned int> o_data_lv3_p0;


  SC_HAS_PROCESS(MergeLV3_p0);

  MergeLV3_p0(sc_module_name n): 
    sc_module(n), 
    tsock("t_skt"), 
    base_offset(0) 
  {
    tsock.register_b_transport(this, &MergeLV3_p0::blocking_transport);
    SC_THREAD(merge_lv3_p0);
  }

  ~MergeLV3_p0() {
	}

	unsigned int sort_array[DATA_NUM_L3];		//SIZE x32
	unsigned int tmp_array[DATA_NUM_L3];
...
// The thread function for the design
    void merge_lv3_p0()
    {
        ...
        while( true )
        {
            ...
            {
                unsigned int in_val = i_data_lv2_p0.read();
                sort_array[n] = in_val;
                // printf("input data =%u\n", in_val);
                // printf("==============\n");
      ...
      case tlm::TLM_READ_COMMAND:
        ...
        switch (addr) {
          case MERGE_SORT_RESULT_ADDR:
            buffer.uint = o_data_lv3_p0.read();
