...
struct MergeLV2_p0 : public sc_module {
  tlm_utils::simple_target_socket<MergeLV2_p0> tsock;
    ...
    //---FIFO IN---
  sc_fifo<unsigned int> i_data_lv1_p0;
  sc_fifo<unsigned int> i_data_lv1_p1;
...
//     //---FIFO OUT---
  ...
  ~MergeLV2_p0() {
	...
	unsigned int sort_array[DATA_NUM_L2];		//SIZE x32
	unsigned int tmp_array[DATA_NUM_L2];
...
// The thread function for the design
    void merge_lv2_p0()
    {
        ...
        while( true )
        {
            ...
            {
                unsigned int in_val = i_data_lv1_p0.read();
                sort_array[n] = in_val;
                // printf("input data =%u\n", in_val);
                // printf("==============\n");
