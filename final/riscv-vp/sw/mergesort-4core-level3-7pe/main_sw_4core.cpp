...
uint32_t barrier3_counter=0, barrier3_lock, barrier3_sem; 
...
static char* const MERGELV1_p2_START_ADDR = reinterpret_cast<char* const>(0x75000000);
static char* const MERGELV1_p2_READ_ADDR  = reinterpret_cast<char* const>(0x75000004);
static char* const MERGELV1_p3_START_ADDR = reinterpret_cast<char* const>(0x76000000);
static char* const MERGELV1_p3_READ_ADDR  = reinterpret_cast<char* const>(0x76000004);
static char* const MERGELV2_p0_START_ADDR = reinterpret_cast<char* const>(0x77000000);
static char* const MERGELV2_p0_READ_ADDR  = reinterpret_cast<char* const>(0x77000004);
static char* const MERGELV2_p1_START_ADDR = reinterpret_cast<char* const>(0x78000000);
static char* const MERGELV2_p1_READ_ADDR  = reinterpret_cast<char* const>(0x78000004);
static char* const MERGELV3_p0_START_ADDR = reinterpret_cast<char* const>(0x79000000);
static char* const MERGELV3_p0_READ_ADDR  = reinterpret_cast<char* const>(0x79000004);
// DMA 
...
int main(unsigned hart_id) {
	...
	if (hart_id == 0) {
		...
		sem_init(&barrier3_sem, 0); //lock all cores initially
...
//--------------------
	for( int count_f = 0 ,fail = 1 ;count_f<FILE_NUM ; count_f++ ){
		...
		for( int i = 0; i < DATA_NUM; i++ )
		{
			...
			// printf("core1: i=%d, value=%u\n", i, value);
				if( i<DATA_NUM_L1 ){
					// printf("sw write data : %u, %d, %d, %d, %d\n", value, buffer[0], buffer[1], buffer[2], buffer[3]);
					// printf("i=%d\n", i);
					// sem_wait(&lock_dma);
					// printf("core0: i=%d, value=%u\n", i, value);
					write_data_to_ACC(MERGELV1_p0_START_ADDR, buffer, 4);			// send the stimulus value
					// sem_post(&lock_dma);
				} else if( i>=DATA_NUM_L1 && i<(2*DATA_NUM_L1) ){
					write_data_to_ACC(MERGELV1_p1_START_ADDR, buffer, 4);			// send the stimulus value
				} else if( i>=(2*DATA_NUM_L1) && i<(3*DATA_NUM_L1) ){
					write_data_to_ACC(MERGELV1_p2_START_ADDR, buffer, 4);			// send the stimulus value
				} else if( i>=(3*DATA_NUM_L1) && i<(4*DATA_NUM_L1) ){
					write_data_to_ACC(MERGELV1_p3_START_ADDR, buffer, 4);			// send the stimulus value
				}
		}
		fclose( infp );
		}
		// sem_wait(&lock_print);
		// printf("arrive barrier0\n");
		// sem_post(&lock_print);
		//======barrier======
		...
		if(hart_id==0){
			...
			for( int i = 0; i < DATA_NUM_L1; i++ ){
				// read_data_from_ACC(MERGELV1_p0_READ_ADDR, buffer, 4);
				// unsigned int outVal = fourCharToInt(buffer[3], buffer[2], buffer[1], buffer[0]) ;
				// write_data_to_ACC(MERGELV2_p0_START_ADDR, buffer, 4);			// send the stimulus value
				// printf("LV1~LV2 core0: %u\n", outVal);
			...
			sem_wait(&lock_lv1_to_lv2);
			for( int i = 0; i < DATA_NUM_L1; i++ ){
				// read_data_from_ACC(MERGELV1_p2_READ_ADDR, buffer, 4);
				// unsigned int outVal = fourCharToInt(buffer[3], buffer[2], buffer[1], buffer[0]) ;
				// write_data_to_ACC(MERGELV2_p1_START_ADDR, buffer, 4);			// send the stimulus value
				// printf("LV1~LV2 core2: %u\n", outVal);
				tran_data_from_ACC_to_ACC(MERGELV1_p2_READ_ADDR, MERGELV2_p1_START_ADDR, 4);
			}
			sem_post(&lock_lv1_to_lv2);
		} else if( hart_id==3 ){
			sem_wait(&lock_lv1_to_lv2);
			for( int i = 0; i < DATA_NUM_L1; i++ ){
				// read_data_from_ACC(MERGELV1_p3_READ_ADDR, buffer, 4);
				// unsigned int outVal = fourCharToInt(buffer[3], buffer[2], buffer[1], buffer[0]) ;
				// write_data_to_ACC(MERGELV2_p1_START_ADDR, buffer, 4);			// send the stimulus value
				// printf("LV1~LV2 core3: %u\n", outVal);
				tran_data_from_ACC_to_ACC(MERGELV1_p3_READ_ADDR, MERGELV2_p1_START_ADDR, 4);
			}
			sem_post(&lock_lv1_to_lv2);
		} 
		...
		if(hart_id==0){
			sem_wait(&lock_lv1_to_lv2);
			for( int i = 0; i < DATA_NUM_L2; i++ ){
				// read_data_from_ACC(MERGELV2_p0_READ_ADDR, buffer, 4);
				// unsigned int outVal = fourCharToInt(buffer[3], buffer[2], buffer[1], buffer[0]) ;
				// write_data_to_ACC(MERGELV3_p0_START_ADDR, buffer, 4);			// send the stimulus value
				// printf("LV2~LV3 core0: %u\n", outVal);
				tran_data_from_ACC_to_ACC(MERGELV2_p0_READ_ADDR, MERGELV3_p0_START_ADDR, 4);
			}
			sem_post(&lock_lv1_to_lv2);
		} else if( hart_id==1 ){
			sem_wait(&lock_lv1_to_lv2);
			for( int i = 0; i < DATA_NUM_L2; i++ ){
				// read_data_from_ACC(MERGELV2_p1_READ_ADDR, buffer, 4);
				// unsigned int outVal = fourCharToInt(buffer[3], buffer[2], buffer[1], buffer[0]) ;
				// write_data_to_ACC(MERGELV3_p0_START_ADDR, buffer, 4);			// send the stimulus value
				// printf("LV2~LV3 core1: %u\n", outVal);
				tran_data_from_ACC_to_ACC(MERGELV2_p1_READ_ADDR, MERGELV3_p0_START_ADDR, 4);
			}
			sem_post(&lock_lv1_to_lv2);
		}

		// sem_wait(&lock_lv1_to_lv2);
		// printf("arrive barrier2\n");
		// sem_post(&lock_lv1_to_lv2);		
		...
		if(hart_id==0){
			...
			for( int i = 0; i < DATA_NUM; i++ )
			{
				read_data_from_ACC(MERGELV3_p0_READ_ADDR, buffer, 4);
				unsigned int outVal = fourCharToInt(buffer[3], buffer[2], buffer[1], buffer[0]) ;
		...
		barrier(&barrier3_sem, &barrier3_lock, &barrier3_counter, PROCESSORS);
		sem_wait(&lock_print);
		printf("pass barrier3\n");
		sem_post(&lock_print);
